...
extension UITableView {
    ...
    ///
    public func dg_heightForCellWithIdentifier(identifier: String, configuration: ((_ cell: UITableViewCell) -> Void)?) -> CGFloat {
        if identifier.characters.count == 0 {
            return 0
        }

        let cell = self.dg_templateCellForReuseIdentifier(identifier: identifier)
        ...
        var contentViewWidth: CGFloat = self.frame.size.width

        // If a cell has accessory view or system accessory type, its content view's width is smaller
        // than cell's by some fixed values.
        if cell.accessoryView != nil {
            contentViewWidth -= 16 + cell.accessoryView!.frame.size.width
        } else {
        ...
        if cell.dg_enforceFrameLayout == true {
            ...
            let inherited: Bool = type(of: cell) == UITableViewCell.self
        ...
        if cell.dg_enforceFrameLayout == true {
            self.dg_debugLog(message: "calculate using frame layout - \(fittingSize.height)")
        } else {
            self.dg_debugLog(message: "calculate using auto layout - \(fittingSize.height)")
        }
    ...
    ///
    public func dg_heightForCellWithIdentifier(identifier: String, key: String, configuration: ((_ cell: UITableViewCell) -> Void)?) -> CGFloat {
        if identifier.characters.count == 0 || key.characters.count == 0 {
            return 0
        }

        // Hit cache
        if self.dg_keyedHeightCache.existsKey(key: key) {
            let height: CGFloat = self.dg_keyedHeightCache[key]!
            self.dg_debugLog(message: "hit cache by key[\(key)] -> \(height)")
            return height
        }

        let height = self.dg_heightForCellWithIdentifier(identifier: identifier, configuration: configuration)
        self.dg_keyedHeightCache[key] = height
        self.dg_debugLog(message: "cached by key[\(key)] --> \(height)")
    ...
    ///
    public func dg_heightForCellWithIdentifier(identifier: String, indexPath: IndexPath, configuration: ((_ cell: UITableViewCell) -> Void)?) -> CGFloat {
        if identifier.characters.count == 0 {
            return 0
        }

        // Hit cache
        if self.dg_indexPathHeightCache.existsIndexPath(indexPath: indexPath) {
            let height: CGFloat = self.dg_indexPathHeightCache[indexPath]!
            self.dg_debugLog(message: "hit cache by indexPath:[\(indexPath.section),\(indexPath.row)] -> \(height)")
            return height
        }

        let height = self.dg_heightForCellWithIdentifier(identifier: identifier, configuration: configuration)
        self.dg_indexPathHeightCache[indexPath] = height
        self.dg_debugLog(message: "cached by indexPath:[\(indexPath.section),\(indexPath.row)] --> \(height)")
    ...
    private func dg_templateCellForReuseIdentifier(identifier: String) -> UITableViewCell {
        ...
        if templateCell == nil {
            ...
            self.dg_debugLog(message: "layout cell created - \(identifier)")
        }
...
extension UITableView {
    ...
    public func dg_insertSections(sections: IndexSet, withRowAnimation animation: UITableViewRowAnimation) {
        if self.dg_indexPathHeightCache.automaticallyInvalidateEnabled {
            self.dg_indexPathHeightCache.insertSections(sections: sections)
        }
        self.dg_insertSections(sections: sections, withRowAnimation: animation)
    }

    public func dg_deleteSections(sections: IndexSet, withRowAnimation animation: UITableViewRowAnimation) {
        if self.dg_indexPathHeightCache.automaticallyInvalidateEnabled {
            self.dg_indexPathHeightCache.deleteSections(sections: sections)
        }
        self.dg_deleteSections(sections: sections, withRowAnimation: animation)
    }

    public func dg_reloadSections(sections: IndexSet, withRowAnimation animation: UITableViewRowAnimation) {
        if self.dg_indexPathHeightCache.automaticallyInvalidateEnabled {
            self.dg_indexPathHeightCache.reloadSections(sections: sections)
        }
        self.dg_reloadSections(sections: sections, withRowAnimation: animation)
    }

    public func dg_moveSection(section: Int, toSection newSection: Int) {
        if self.dg_indexPathHeightCache.automaticallyInvalidateEnabled {
            self.dg_indexPathHeightCache.moveSection(section: section, toSection: newSection)
        }
        self.dg_moveSection(section: section, toSection: newSection)
    }

    public func dg_insertRowsAtIndexPaths(indexPaths: [IndexPath], withRowAnimation animation: UITableViewRowAnimation) {
        if self.dg_indexPathHeightCache.automaticallyInvalidateEnabled {
            self.dg_indexPathHeightCache.insertRowsAtIndexPaths(indexPaths: indexPaths)
        }
        self.dg_insertRowsAtIndexPaths(indexPaths: indexPaths, withRowAnimation: animation)
    }

    public func dg_deleteRowsAtIndexPaths(indexPaths: [IndexPath], withRowAnimation animation: UITableViewRowAnimation) {
        if self.dg_indexPathHeightCache.automaticallyInvalidateEnabled {
            self.dg_indexPathHeightCache.deleteRowsAtIndexPaths(indexPaths: indexPaths)
        }
        self.dg_deleteRowsAtIndexPaths(indexPaths: indexPaths, withRowAnimation: animation)
    }

    public func dg_reloadRowsAtIndexPaths(indexPaths: [IndexPath], withRowAnimation animation: UITableViewRowAnimation) {
        if self.dg_indexPathHeightCache.automaticallyInvalidateEnabled {
            self.dg_indexPathHeightCache.reloadRowsAtIndexPaths(indexPaths: indexPaths)
        }
        self.dg_reloadRowsAtIndexPaths(indexPaths: indexPaths, withRowAnimation: animation)
    }

    public func dg_moveRowAtIndexPath(indexPath: IndexPath, toIndexPath newIndexPath: IndexPath) {
        if self.dg_indexPathHeightCache.automaticallyInvalidateEnabled {
            self.dg_indexPathHeightCache.moveRowAtIndexPath(indexPath: indexPath, toIndexPath: newIndexPath)
        }
        self.dg_moveRowAtIndexPath(indexPath: indexPath, toIndexPath: newIndexPath)
    }
}
